...
**Note (this document's own ADR list below is stale — stops at ADR-017; ADR-018 through ADR-022 exist in `docs/adr/` but are not yet reflected here, a pre-existing gap):** four small, previously-open items were closed after this note was added — (1) the StatsBomb brand-logo attribution requirement (ADR-020 clause 1.4) is now visible in `dashboard.py`'s sidebar and in `README.md`, unconditionally (not gated by `PUBLIC_DEPLOYMENT`); (2) `production/src/serving/api.py` gained a single, optional `X-API-Key` header check (off by default — see ADR-022 for why a full auth system was deliberately NOT built); (3) `api.py` gained `GET /health` and `GET /metrics` (real, non-hardcoded operational status/counters); (4) `dashboard.py` gained a sixth tab, "Alerts History" — a read-only, filterable (match_id/source/date-range) browser over ADR-019's `GET /alerts/history` endpoint, the first UI surface for that persistence store (previously API-only).
...
| Live serving & dashboard | WebSocket/REST API, Streamlit dashboard, per-connection isolation, async non-blocking CV integration (M16-19, M33); ADR-019's alert-history store now has a real UI (dashboard's "Alerts History" tab: filterable by match_id/source/date-range, health-gated error handling, real data validated end-to-end) | Real calibration wired through the live CV API path (currently pixel-space only, not physically meaningful yet) |
